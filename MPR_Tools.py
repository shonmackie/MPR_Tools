...
class acceptance:
    '''
    conversion foil upon which neutrons impinge and scatter protons
    '''
    def __init__(self, rf, T, L, ra, srim_path, sigma_np_path, diffxs_path, Nrf=1000, Naf=1000, Nzf=1000):
        '''
        rf - cm, foil radius
        T - um, foil thickness
        ...
        print('loaded elastic scattering cross sections from ', sigma_np_path)
    ...
    def SRIM(self, E0, L, N=100):
        ...
        E=E0
        for i in range(N):
            E-=self.SP(E)*dL*1e3 #SP is in MeV/mm
        return E
    ...
    def diff_xs_LAB(self, E, theta):
        '''
        theta is in the center of mass frame
    ...
    def generate_ray(self, E, Kinematic=False, SRIM=False, N_s=1000):
        '''
        Generate a proton ray scattered by a neutron with normal incidence. Scatter grid limited from 0 to pi/6 to improve calculation efficiency (dont make protons you know don't enter)
        ...
        '''
        lim=np.arctan((self.rf+self.ra)/self.L) #max angle that is accepted
        scatter_grid = np.linspace(0, lim*1.1, N_s) #don't sample beyond what could possibly be accepted
        ...
        x0=rf*np.cos(thetaf)
        y0=rf*np.sin(thetaf)
        Pz=np.exp(-(self.zf_grid+self.T)*self.sigma_np_total(E)*self.np)
        z0=self.T*np.random.choice(self.zf_grid, p=Pz/np.sum(Pz))
        ...
        if Kinematic:
            E=E*np.cos(theta_s)
        if SRIM:
            L_SRIM = (self.T-z0)/np.cos(theta_s)
    ...
    def get_efficiency(self, E, N=int(1e5), N_s=1000):
        '''
        estimate the intrinsic efficiency of the spectrometer 
        ...
        tot=N
        acpt=0
        eps_scat = (1-np.exp(-self.np*self.sigma_np_total(E)*self.T)) #fraction of neutrons which undergo a scattering interaction on protons in the foil (assuming normal incidence)
        ...
        for n in range(tot):
            ...
            xa=x0+self.L*np.tan(theta_s)*np.cos(phi_s)
            ya=y0+self.L*np.tan(theta_s)*np.sin(phi_s)
            if xa**2+ya**2<=self.ra**2:
                acpt+=1
        eps_acpt = acpt/tot
        return eps_acpt*eps_scat
...
class MPR:
    ...
    def GenRays(self, E, f_E, Np, kinematics=False, SRIM=False):
        '''
        ...
        '''
        print('generating', Np, 'initial proton trajectories...')
        ...
        while n<Np-1:
            ...
            x0, y0, th0, ph0, e = self.acceptance.generate_ray(e, kinematics, SRIM)
            ...
            N+=1
        print('sampled', N, 'scattering events to generate', Np, 'accepted protons. Scattering calculation efficiency:', str(Np/N))
    ...
    def plot_output_XY(self, draw_hodoscope=False):
        fig, ax = plt.subplots(1,1, figsize=(8,4))
        ax.set_title('Proton Positions')
        if draw_hodoscope:
        ...
        a = ax.scatter(self.beam_out[:,0], self.beam_out[:,2], s=0.6, c=self.beam_in[:,4]*self.refE+self.refE, cmap='jet')
        fig.colorbar(a, label = 'proton energy [MeV]')
        ax.legend()
        ax.set_xlabel('horizontal position [m]')
        ax.set_ylabel('vertical position [m]')   
        plt.savefig('XYScatter_out')
    ...
    def assess_monoenergetic_performance(self, E, N=10000, kinematics = False, SRIM=False):
        '''
