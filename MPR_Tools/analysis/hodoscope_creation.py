...
class HodoscopeCreator:
    ...
    * From the bottom of that line, a horizontal line extends to the left until it
    ...
    def generate_bins(
        ...
        while True:
            # Next bin: its lower edge equals the current upper edge
        ...
        while True:
            # Previous bin: its upper edge equals the current lower edge
    ...
    def plot_bins(
        ...
        # Interpolators for drawing the staircase
        position_of_energy = interp1d(energies, positions, bounds_error=False,
                                      fill_value=(positions[0], positions[-1]))
        ...
        for i, (bin_energy, bin_position) in enumerate(zip(bin_energies, bin_positions)):
            ...
            # Horizontal line from top of current bin to next bin's center energy (rightward)
            if i < len(bin_energies) - 1:
                next_bin_energy = bin_energies[i + 1]
                ax.plot([bin_energy, next_bin_energy], [bin_top, bin_top],
                        color=staircase_color, linewidth=1.2, alpha=0.8)

            # Horizontal line from bottom of current bin to previous bin's center energy (leftward)
            if i > 0:
                prev_bin_energy = bin_energies[i - 1]
                ax.plot([prev_bin_energy, bin_energy], [bin_bottom, bin_bottom],
                        color=staircase_color, linewidth=1.2, alpha=0.8)
